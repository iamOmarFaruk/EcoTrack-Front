            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <div className="h-2 w-8 rounded-full bg-indigo-400/40" />
                        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-indigo-500">Event Coordination</p>
                    </div>
                    <h1 className="text-3xl md:text-4xl font-extrabold text-heading tracking-tight">
                        Events <span className="text-indigo-500">Management</span>
                    </h1>
                ...
                <div className="flex items-center gap-3">
                    <div className="relative group">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-text/30 group-focus-within:text-indigo-500 transition-colors" size={18} />
                        <input
                            type="text"
                            placeholder="Search events..."
                            className="pl-10 pr-4 py-2.5 rounded-xl border border-border bg-surface/50 backdrop-blur-sm text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all w-64"
                        />
                    </div>
                </div>
            </div>
                ...
                {events.length > 0 ? (
                    events.map((event, index) => (
                        <motion.div
                            ...
                            key={event._id}
                            className="group relative overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900/50 p-5 transition-all hover:shadow-xl hover:shadow-indigo-500/5 hover:border-indigo-500/20"
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="flex items-start gap-4 flex-1">
                                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 group-hover:bg-indigo-500/10 transition-colors">
                                        <Calendar className="text-indigo-500/40 group-hover:text-indigo-500 transition-colors" size={24} />
                                    </div>
                                    <div className="min-w-0">
                                        <h3 className="text-lg font-bold text-heading group-hover:text-indigo-500 transition-colors truncate">{event.title}</h3>
                                        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px] font-medium">
                                            <span className={clsx(
                                                ...
                                                statusBadges[event.status] || 'bg-slate-100 text-slate-600'
                                            )}>
                                                {event.status}
                                            </span>
                                            ...
                                            <span className="flex items-center gap-1.5 text-text/50">
                                                <Clock size={14} className="text-indigo-500/60" />
